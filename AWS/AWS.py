    ...
    def download(cls, bucket_name, file_name):
        ...
        try:
            with open(file_name, "wb") as file:
                file = s3_client.download_fileobj(bucket_name, file_name, file)
                return ("file", file)
    ...
    def upload(cls, file, bucket_name, file_name, ext):
        ...
        try:
            response = s3_client.upload_fileobj(
                file, bucket_name, file_name, ExtraArgs={"ContentType": "image/jpeg"}
            )
            print(f"AWS upload response: {response}")
            return True

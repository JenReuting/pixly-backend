...
def log_request():
    ...
    print(f" -----> BACKEND API - INCOMING REQUEST -----> {request}")
...
def upload_image():
    ...
    TODO: This should return the signed image url, not the database instance"""
    ...
    print(
        f" -----> APP - POST /upload -----> file: {file} title: {title} description: {description}"
    )
    ...
    print(f" -----> BACKEND API - OUTGOING RESPONSE ----->  image created: {image}")
...
def get_image(id):
    ...
    print(f" -----> BACKEND API - GET /:file_name -----> id: {id}")
    try:
        image = Image.query.get_or_404(id)
